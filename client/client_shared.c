...
int client_config_load(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[]) {
    ...
    if ((cfg->tls_engine_kpass_sha1 && (!cfg->keyform || !cfg->tls_engine))) {
        fprintf(stderr,
                "Error: when using tls-engine-kpass-sha1, both tls-engine and keyform must also be provided.\n");
        return 1;
    }
#endif
#ifdef FINAL_WITH_TLS_PSK
    ...
    if (cfg->protocol_version == 5) {
        ...
        if (cfg->session_expiry_interval > 0) {
            ...
            rc = mosquitto_property_add_int32(&cfg->connect_props, MQTT_PROP_SESSION_EXPIRY_INTERVAL,
                                              (uint32_t) cfg->session_expiry_interval);
            if (rc) {
